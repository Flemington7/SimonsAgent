...
class LlamaForSequenceClassification(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        logits = self.classifier(outputs.last_hidden_state[:, 0, :])  # 取CLS token
        loss = None
...
def main(**kwargs):
    ...
    if train_config.enable_fsdp and train_config.low_cpu_fsdp:
        ...
        if rank == 0:
            model = LlamaForSequenceClassification(
                pretrained_model_name=train_config.model_name, 
                num_labels=3  # 假设有3个分类
            )
        else:
            llama_config = LlamaConfig.from_pretrained(train_config.model_name)
            llama_config.use_cache = use_cache
            with torch.device("meta"):
                model = LlamaForSequenceClassification(llama_config, num_labels=3)
    ...
    if len(tokenizer) > model.llama.get_input_embeddings().weight.shape[0]:
